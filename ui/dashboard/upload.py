...
class Upload:
    ...
    @staticmethod
    def process_and_upload_file(file_path, collection_name):
        pdf_content = st.session_state.Data.process_pdf(file_path)
        model = st.session_state.embedding_model
        st.session_state.qdrant_db.upload_data(collection_name, pdf_content, model)
    ...
    def show():
        ...
        st.info(f"Lưu ý: Kích thước file tối đa là {Upload.MAX_FILE_SIZE_MB}MB.")

        uploaded_file = st.file_uploader(f"Chọn file {Upload.ALLOWED_FILE_TYPE}", type=Upload.ALLOWED_FILE_TYPE,
        ...
        if uploaded_file:
            ...
            if uploaded_file.size > Upload.MAX_FILE_SIZE_BYTES:
                st.error(f"File quá lớn. Vui lòng tải lên file có kích thước nhỏ hơn {Upload.MAX_FILE_SIZE_MB}MB.")
            else:
                    ...
                    with st.form("new_database_form"):
                        ...
                        if submit_new:
                            if new_collection_name.strip() in st.session_state.collection_names:
                                ...
                                Upload.process_upload(uploaded_file, new_collection_name.strip())
                ...
                with tab2:
                    with st.form("existing_database_form"):
                        ...
                        if submit_existing:
                            ...
                            Upload.process_upload(uploaded_file, collection_name)

                        if refresh_existing:
                            st.rerun()

    @staticmethod
    def process_upload(uploaded_file, collection_name):
        temp_filename = Upload.get_temp_filename(collection_name)
        try:
            ...
            with st.spinner('Đang xử lý...'):
                ...
                try:
                    Upload.process_and_upload_file(temp_filename, collection_name)
